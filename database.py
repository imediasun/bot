...
async def create_agent(user_id):
    ...
    if not user:
        cur.execute("INSERT INTO info_player VALUES(?,?,?,?,?,?,?,?,?,?,?)", (user_id, '','','','','','','','','',''))
    db.commit()

async def edit_agent(state, user_id):
    async with state.proxy() as data:
        cur.execute("""UPDATE info_player SET nickname = ?, age = ?, teamspeak = ?, teamspeak1 = ?, device = ?, tournament = ?, finals = ?, highilights = ?, description = ?, contact = ? WHERE user_id = ?""",
                    (data['nickname'], data['age'], data["teamspeak"], data["teamspeak1"], data['device'], data['tournament'], data['finals'], data['highilights'], data['description'], data['contact'], user_id))
        db.commit()
...
async def update_team_profile(state, user_id):
    async with state.proxy() as data:
        cur.execute("""UPDATE info_team SET team_name = ?, age = ?, teamspeak = ?, teamspeak = ?, role = ?, device = ?, tournament = ?, finals = ?, description = ?, contact = ? WHERE user_id = ?""",
                    (data['team_name'], data['age'], data["teamspeak"],  data["teamspeak1"], data['role'], data['device'], data['tournament'], data['finals'], data['description'], data['contact'], user_id))
    db.commit()
...
async def create_tour(state, user_id, tour_id):
    async with state.proxy() as data:
        cur.execute("INSERT INTO info_tour (tour_id, user_id, format, photo, desc, url) VALUES(?, ?, ?, ?, ?, ?)", (tour_id, user_id, data['format'],data['photo'], data['desc'], data['url'],))
    db.commit()
...
# ADMIN VIP SLOTS
async def create_vip_slots(state, user_id, tour_id):
    async with state.proxy() as data:
        cur.execute("INSERT INTO vip_slots (tour_id, user_id, stage, photo, desc, price, tour_name, ds_link, time, amount, amount2) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (tour_id, user_id, data['stage'],data['photo'], data['desc'], data['price'], data['tour_name'], data['link'], data['time'], data['amount'], data['amount']))
    db.commit()
...
async def update_vip_slots(state, user_id):
    async with state.proxy() as data:
        cur.execute("UPDATE vip_slots SET user_id = ?, stage = ?, photo = ?, desc = ?, price = ?, tour_name = ?, time = ?, amount = ?, amount2 = ? WHERE tour_id = ?",
                    (user_id, data['stage'], data['photo'], data['desc'], data['price'], data['tour_name'], data['time'], data['amount'], data['amount'], data['tour_id']))
    db.commit()
...
async def get_vip_slots(stage):
    return cur.execute("SELECT * FROM vip_slots WHERE stage = ? AND time != '///'", (stage,)).fetchall()
...
async def get_time_vip_slot(tour_id):
    return cur.execute("SELECT time FROM vip_slots WHERE tour_id = ?", (tour_id,)).fetchone()[0]
...
# PAY VIP SLOT
async def create_pay_vip_slot(pay_id, user_id, username):
    cur.execute("INSERT INTO buy_vip_slot(id, user_id, username) VALUES(?, ?, ?)", (pay_id, user_id, username))
    db.commit()

async def update_pay_vip_slot(pay_id, pay_number, tour_id, bank, amount, currency, time):
    cur.execute("UPDATE buy_vip_slot SET amount = ?, currency = ?, bank = ?, pay_number = ?, vip_slot_id = ?, time = ? WHERE id = ?",
...
async def create_events(state, tour_id,  user_id):
    async with state.proxy() as data:
        cur.execute("INSERT INTO events (tour_id, user_id, time, photo, price, desc, tour_name, link, amount, amount2) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (tour_id, user_id, data['time'], data['photo'], data['price'], data['desc'], data['tour_name'], data['link'], data['amount'], data['amount']))
    db.commit()

async def update_event(state, user_id):
    async with state.proxy() as data:
        cur.execute("UPDATE events SET user_id = ?, time = ?, photo = ?, price = ?, desc = ?, tour_name = ?, link = ?, amount = ?, amount2 = ? WHERE tour_id = ?",
                    (user_id, data['time'], data['photo'], data['price'], data['desc'], data['tour_name'], data['link'], data['amount'], data['amount'], data['tour_id']))
    db.commit()
...
async def create_payment(id, user_id, user_name):
    cur.execute("INSERT INTO buy_uc(id, user_id, user_name, proof, proof_d, amount, currency, amount_uc, bank, pay_number) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
...
async def update_pay_info(state):
    async with state.proxy() as data:
        cur.execute("UPDATE buy_uc SET game_id = ?, nick = ? WHERE id = ?", (data['game_id'], data['nick'], data['uc_id']))
    db.commit()
...
async def update_admin(state, user_id):
    async with state.proxy() as data:
        cur.execute("UPDATE admin_table SET admin_id = ?, user_id = ?, admin_username = ?, job_title = ?, type = ? WHERE admin_id = ?",
...
async def get_admin_id(admin_id):
    admin = cur.execute("SELECT admin_id FROM admin_table WHERE type = 'ADMIN' AND admin_id = ?", (admin_id,)).fetchone()
    if not admin:
        return []
    else:
        return admin
...
async def get_moder_id(admin_id):
    moder = cur.execute("SELECT admin_id FROM admin_table WHERE admin_id = ?", (admin_id,)).fetchone()
    if not moder:
        return []
    else:
        return moder
...
async def create_news(state, user_id, news_id):
    async with state.proxy() as data:
        cur.execute("INSERT INTO news(news_id, user_id, text, url) VALUES(?, ?, ?, ?)", (news_id, user_id, data['text'], data['url']))
    db.commit()
...
async def create_vip_prac(state, tour_id, user_id):
    async with state.proxy() as data:
        cur.execute("INSERT INTO prac_vip_slot(tour_id, user_id, time, photo, desc, tour_name, price, link, amount, amount2) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    (tour_id, user_id, data['time'], data['photo'], data['desc'], data['tour_name'], data['price'], data['link'], data['amount'], data['amount']))
        db.commit()

async def update_vip_prac(state, user_id):
    async with state.proxy() as data:
        cur.execute("UPDATE prac_vip_slot SET user_id = ?, time = ?, photo = ?, desc = ?, tour_name = ?, price = ?, link = ?, amount = ?, amount2 = ? WHERE tour_id = ?",
                    (user_id, data['time'], data['photo'], data['desc'], data['tour_name'], data['price'], data['link'], data['amount'], data['amount'], data['tour_id']))
    db.commit()
...
async def create_prac_agent(user_id):
    ...
    if not user:
        cur.execute("INSERT INTO young_player VALUES(?,?,?,?,?,?,?,?,?,?)", (user_id, '','','','','','','','',''))
    db.commit()

async def edit_prac_agent(state, user_id):
    async with state.proxy() as data:
        cur.execute("""UPDATE young_player SET nickname = ?, age = ?, teamspeak = ?, teamspeak1 = ?, device = ?, practice_game = ?, highilights = ?, description = ?, contact = ? WHERE user_id = ?""",
                    (data['nickname'], data['age'], data["teamspeak"], data["teamspeak1"], data['device'], data['practice_games'], data['highilights'], data['description'], data['contact'], user_id))
        db.commit()
...
async def edit_prac_team(state, user_id):
    async with state.proxy() as data:
        cur.execute("""UPDATE young_team SET team_name = ?, age = ?, teamspeak = ?, teamspeak1 = ?, role = ?, device = ?, practice_game = ?, description = ?, contact = ? WHERE user_id = ?""",
                    (data['team_name'], data['age'], data["teamspeak"], data["teamspeak1"], data['role'], data['device'], data['practice_games'], data['description'], data['contact'], user_id))
        db.commit()
